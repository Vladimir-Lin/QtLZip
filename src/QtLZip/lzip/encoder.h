...
public:
  ...
  Len_prices( const LZipLib::LenModel & m, const int match_len_limit )
    :
    lm( m ),
...
class LZ_encoder : public LZ_encoder_base
  ...
  int price_shortrep( const LZipLib::State state, const int pos_state ) const
    {
    return price0( bm_rep0[state()] ) + price0( bm_len[state()][pos_state] );
    }

  int price_rep( const int rep, const LZipLib::State state, const int pos_state ) const
    {
  ...
  int price_rep0_len( const int len, const LZipLib::State state, const int pos_state ) const
    {
